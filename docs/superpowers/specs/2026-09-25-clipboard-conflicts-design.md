...
- Drop ke drive yang belum di-mount di sidebar (diabaikan). Progress/batal.
...
## Clipboard sistem (revisi setelah test manual)

Clipboard internal diganti clipboard sistem (`gdk::Display::clipboard()`), jadi copy/paste antar aplikasi berfungsi dua arah:

- Ctrl+C / Ctrl+X menulis satu `gdk::ContentProvider` gabungan: `gdk::FileList` (GTK men-serialisasi ke `text/uri-list`), `x-special/gnome-copied-files` berisi `paste::gnome_copied_files(paths, cut)` (dipakai Nautilus/Dolphin untuk tahu copy vs cut), dan `text/plain` berisi path per baris.
- Ctrl+V membaca clipboard secara async (UI tidak blocking): utamakan `x-special/gnome-copied-files` (`paste::parse_gnome_copied_files`, tahu cut), lalu `gdk::FileList` (dianggap copy). Isi lain (teks biasa, gambar mentah) → diabaikan.
- Setelah cut-paste tanpa item gagal, clipboard sistem dikosongkan (seperti Nautilus). `App.clipboard` internal dihapus.
- Format:
  - `gnome_copied_files(paths, cut) -> String`: `copy` atau `cut`, lalu satu URI `file://` per path (`gio::File::uri`), dipisah `\n`, tanpa newline di akhir.
  - `parse_gnome_copied_files(&str) -> Option<(Vec<PathBuf>, bool)>`: baris pertama `copy`/`cut`; toleran `\r\n` dan baris kosong; URI non-`file://` dilewati; tidak ada path tersisa atau baris pertama lain → `None`.

## Drop ke Trash (revisi)

Drop ke baris Trash di sidebar (dari list atau aplikasi lain) → plan `Trash` per item → `Msg::Submit(Job::Trash, ..)`, jadi lewat Validator dan dialog konfirmasi trash yang sudah ada (R8).

## Bug: copy lalu paste di folder yang sama (revisi)

Validator menolak `Copy { src, dst }` dengan `dst == src` sebagai `IntoItself`, sehingga "Keep Both" tidak pernah jalan. Aturan baru: `dst` sama dengan `src` hanya boleh dengan `ConflictPolicy::KeepBoth` (executor memilih `name (2)`); Skip/Replace/tanpa kebijakan tetap `IntoItself` (Replace akan men-trash sumbernya sendiri). `dst` di dalam `src` tetap selalu ditolak. Test: `tests/validator.rs::copying_onto_itself_needs_keep_both`.
...
## Unit baru: `anchoa::paste` (`src/paste.rs`)
...
- Tidak ada state clipboard di `App`; sumbernya clipboard sistem.
